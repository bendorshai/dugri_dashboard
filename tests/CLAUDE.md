...
When the production code path changes (e.g. `handle_message` switches from `route_message` to `classify_message`), ALL LLM test files that test routing/classification MUST be updated to call the same method. Tests that call the old method are testing dead code and give false confidence.
