...
class TestShouldRetryTarget:
    def test_true_on_day_9_when_dormant(self):
        svc, _ = _make_service()
        user = _make_user(
            trial_started_at=datetime.now(timezone.utc) - timedelta(days=9),
        )
        assert svc.should_retry_target(user, day_number=9) is True

    def test_false_if_already_retried(self):
        svc, _ = _make_service()
        user = _make_user(
            trial_started_at=datetime.now(timezone.utc) - timedelta(days=9),
            target_retry_done=True,
        )
        assert svc.should_retry_target(user, day_number=9) is False

    def test_false_if_cancelled(self):
        svc, _ = _make_service()
        user = _make_user(
            trial_started_at=datetime.now(timezone.utc) - timedelta(days=9),
            toggles=Toggles(target_data=ToggleState(status="cancelled")),
        )
        assert svc.should_retry_target(user, day_number=9) is False

    def test_false_if_active(self):
        svc, _ = _make_service()
        user = _make_user(
            trial_started_at=datetime.now(timezone.utc) - timedelta(days=9),
            toggles=Toggles(target_data=ToggleState(status="active")),
        )
        assert svc.should_retry_target(user, day_number=9) is False


class TestShouldRetryEatingWindow:
    def test_true_after_retry_days(self):
        svc, _ = _make_service()
        user = _make_user(
            toggles=Toggles(eating_window=ToggleState(
                revealed_at=datetime.now(timezone.utc) - timedelta(days=12),
            )),
        )
        assert svc.should_retry_eating_window(user) is True
    ...
    def test_false_if_already_retried(self):
        ...
        assert svc.should_retry_eating_window(user) is False
    ...
    def test_false_if_not_yet_revealed(self):
        svc, _ = _make_service()
        user = _make_user()
        assert svc.should_retry_eating_window(user) is False
